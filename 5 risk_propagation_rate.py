...
def plot_sir_by_beta(graph, source_sets, betas, gammas, steps, intervention_steps, node_names, simulations):
    ...
    for idx, (sources, beta, gamma, step_point) in enumerate(zip(source_sets, betas, gammas, intervention_steps)):
        ...
        plt.plot(x, avg_result, label=f'β = {beta}')
    ...
    plt.text(-steps * 0.05, -0.22,
             f'Default parameters: Initial infection node: {sources[0]}, ∆s = {intervention_steps[0]}, γ = {gammas[0]}',
             fontsize=14)
    plt.figtext(0.5, -0.1, "c) Different risk propagation rates", ha='center', fontsize=16)
    plt.show()
...
if __name__ == '__main__':
    ...
    plot_sir_by_beta(graph, first_i, betas, gammas, steps, intervention_steps, node_names, simulations)